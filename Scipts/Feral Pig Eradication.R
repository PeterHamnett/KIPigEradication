...
harv3plot <- ggplot(data = minn.prop.pop3, mapping = aes(x=harv.prop.consist)) +
  ...
  geom_vline(xintercept = 0.7, linetype = 3, color = "black") +
  theme_bw() +
...
harv10plot <- ggplot(data = minn.prop.pop, mapping = aes(x=harv.prop.consist)) +
  ...
  geom_vline(xintercept = 0.5, linetype = 3, color = "black") +
  theme_bw() +
  labs(x = "Harvest Proportion", y = "Min N")
...
for (s in 1:length(harv.prop.consist)) {
    ...
    for (e in 1:iter) {
      ...
      for (i in 1:t) {
        ...
        hrsPigTAAC <- as.numeric(coef(fitTAAC)[1] + (coef(fitTAAC)[2] * log(prop.remain)))
        hrsPigShot <- as.numeric(coef(fitShot)[1] + (coef(fitShot)[2] * log(prop.remain)))
        hrsPigTrap <- as.numeric(coef(fitTrap)[1] + (coef(fitTrap)[2] * log(prop.remain)))
        hrsPoison <- as.numeric(coef(fitPoison)[1] + (coef(fitPoison)[2] * log(prop.remain)))
...
p3C<-ggplot(scenarios.tot.df, aes(x = harvest.prop, y = tot.cost.med, group=scenario, colour=scenario)) +  
  ...
  labs(title = "", subtitle =  "", y = "", x ="") + geom_vline(xintercept = 0.7, linetype = 3, color = "black") +
